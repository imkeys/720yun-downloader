...
class Crawler:
    __time_sleep = 0.1
    __dir = './download/' + datetime.datetime.now().strftime('%Y%m%d%H%M%S') + '/'
    __referrer = 'https://www.720yun.com'

    def __init__(self, t=0.1):
        self.time_sleep = t

    def getSuffix(self, url):
        m = re.search(r'\.[^\.]*$', url)
    ...
    def saveImage(self, imgurl):
        ...
        fileName = self.getFileName(imgurl)
        # 指定UA和referrer，减少403
        ...
        urllib.request.urlretrieve(imgurl, self.__dir + str(fileName))
